...
void saveGrayscaleBMP(const char* filename, const uint8_t* pixels, int width, int height) {
...
bool load24BitBMPFromBuffer(const uint8_t* data, size_t len, uint8_t** out_pixels, int* width, int* height) {
    if (len < sizeof(BMPHeader) + sizeof(DIBHeader)) return false;

    const BMPHeader* bmp = (const BMPHeader*)data;
    const DIBHeader* dib = (const DIBHeader*)(data + sizeof(BMPHeader));

    if (bmp->bfType != 0x4D42 || dib->biBitCount != 24 || dib->biCompression != 0) return false;
...
const uint8_t bmp_inline[] = {
 ...
 int main(int argc, char** argv) {
    ...
    dim3 dimBlock(16, 16);
    colortoGrayscaleConvertion<<<dimGrid, dimBlock>>>(Pout_d, Pin_d, width, height);

    cudaMemcpy(pixels_out, Pout_d, size, cudaMemcpyDeviceToHost);

    saveGrayscaleBMP("out.bmp", pixels_out, width, height);

    std::cout << "First pixel grayscale: " << (int)pixels_out[0] << "\n";
