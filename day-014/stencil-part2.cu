...
__global__ void stencil_kernel_coarse(float* in, float* out, unsigned int n) {
...
int main() {
  ...
  start = std::chrono::high_resolution_clock::now();
  stencil_kernel_coarse<<<grid, block>>>(d_input, d_output, N);
  cudaMemcpy(h_output, d_output, total_bytes, cudaMemcpyDeviceToHost);
  end = std::chrono::high_resolution_clock::now();

  elapsed = end - start;
  std::cout << std::fixed << std::setprecision(3)
            << "coarse thread: " << elapsed.count() << " ms" << std::endl;
